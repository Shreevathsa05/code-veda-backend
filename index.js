...
import {
  ...
  HireModel,
  ApplicationModel,
} from "./src/mongodb/CommunityEvents.js";
...
app.get("/events", async (req, res) => {
  try {
    const now = new Date();
    const events = await EventsModel.find({ date: { $gt: now } }).sort({
      date: 1,
    });
    res.json(events);
  } catch (err) {
...
app.get("/local-alerts", async (req, res) => {
  try {
    const now = new Date();
    const alerts = await LocalAlerts.find({ date: { $gte: now } }).sort({
      date: 1,
    });
    res.json(alerts);
  } catch (err) {
...
app.get("/local-alerts/:id", async (req, res) => {
  try {
    ...
    if (!alert)
      return res.status(404).json({ message: "Local alert not found" });
    res.json(alert);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put("/local-alerts/:id", async (req, res) => {
  try {
    const updatedAlert = await LocalAlerts.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
// CREATE
app.post("/hire", async (req, res) => {
  console.log("Received new job posting data:", req.body); // <-- ADD THIS LINE
  try {
    const hire = new HireModel(req.body);
    const saved = await hire.save();
    console.log("Job posting saved successfully:", saved); // <-- ADD THIS LINE
    res.status(201).json(saved);
  } catch (err) {
    console.error("Error saving job posting:", err.message); // <-- ADD THIS LINE
    res.status(400).json({ error: err.message });
  }
});
...
app.put("/hire/:id", async (req, res) => {
  try {
    const updated = await HireModel.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    res.json(updated);
  } catch (err) {
